...
const BLURBS: Record<Mode | ActionGroup, string> = {
  ...
  notes: 'One page of free text for this project, kept in .dashboard/notes.md beside the board, so it '
    ...
    + 'were never shown the file in must not be saved over it. Type into it anyway and it is '
    + 'stuck that way — reading now would take what you typed off the screen, so no later '
    + 'arrival reads either. The bar says so on every arrival; copy the text out, and the file '
