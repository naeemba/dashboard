...
const BLURBS: Record<Mode | ActionGroup, string> = {
  ...
  manager: 'The first tab, and the only page that is not a project: no folder and no shells, so the '
    ...
    + 'together. The last line on the page is the version of the build you are running, there to be '
    + 'held up against the version on main: the same number and the copy you have installed has '
    + 'everything that has been merged. Enter on a pane takes you '
