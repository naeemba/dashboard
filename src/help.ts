...
const BLURBS: Record<Mode | ActionGroup, string> = {
  ...
  board: 'A kanban board kept in .dashboard/board.json inside the project. Every change is written '
    ...
    + 'project, and starts an agent in a pane that is sitting at a prompt with nothing running in it. '
    + 'A pane still standing in an earlier card\'s worktree is taken last, since its transcript is '
    + 'still on screen. If every pane is running something the ship says so and names them. '
    + 'Only what is in the foreground counts as running, and the pane it takes is restarted in the '
    + 'worktree: a line you typed but never submitted goes with it, and so does a job you put in the '
    + 'background — a pane where you ran npm run dev & is a pane at a prompt as far as the ship can '
    + 'see. Leave a long job in the foreground and the ship steps around it. '
    + 'That last part is unix only: on Windows nothing about the foreground can be read, so a pane is '
    + 'held back only while an agent is working in it and a job of yours in the foreground is taken '
    + 'like any other. '
    + 'Nothing you have written on '
    ...
    + 'around. '
    + 'Moving a card into Review yourself does nothing: it is where the app puts things, not a key. '
    + 'The card lands in Review either way, and carries a comment when the worktree could not be '
    + 'swapped, since nothing was waiting on screen to be told. Something left uncommitted in it is '
    + 'the one of those that mends itself: git will not give up a worktree with a change in it, so the '
    + 'app asks again every few seconds and the review starts on its own the moment that change is '
    + 'committed. Every other reason is asked once and the comment is the end of it. '
    + 'A board too long to read is searched rather than walked: / opens a box over it, typing narrows '
    + 'the list to the cards whose titles match and then the cards with the word written on them '
    + 'somewhere — in a description or in the trail, with the line it was found on beside the title — '
    + 'and Enter puts the selection on the one you picked, where every other key works as it always '
    + 'does. Opening it and typing nothing is the whole board as one list. '
  ...
  manager: 'The first tab, and the only page that is not a project: no folder and no shells, so the '
    ...
    + 'this morning. Every project also carries three token figures at the end of its row, in the '
    + 'three columns named above the list: the last five hours, this week counting from Monday, and '
    + 'all time. A pane running a Claude Code agent carries what that agent has cost, under the last '
    + 'of the three. They are read out of Claude Code\'s own logs on this machine and refresh every '
    + 'half minute, and the line along the foot of the page adds every open project\'s three '
    + 'together. The last line on the page is the version of the build you are running, which is '
    + 'there to be held up against the version in package.json on main: a smaller number means the '
    + 'copy in your Applications folder is behind what has been merged and wants rebuilding. Enter on a pane takes you '
  ...
  command: 'One command, run in the projects you mark. Type it once — `npm audit`, `npm outdated`, '
    ...
    + 'has not died and has nothing running in it: it is the same question a ship asks before it takes '
    + 'a pane, and gets the same answer, so a pane a ship would step around is one this will not type '
    + 'into either. An agent, a dev server, a tail, psql, vim — all of them hold a pane back. It picks '
    + 'in the ship\'s order too, which is the lowest-numbered free pane, except that a pane still '
    + 'standing in a finished card\'s worktree goes last — otherwise the line runs in that card\'s '
    + 'checkout while an empty prompt of the project sits below it. '
    + 'The one thing it cannot see is a job you put in the background: run npm run dev & and the shell '
    + 'is at its prompt again, so that pane takes the line like any other. '
    + 'That last part is unix only, the same way the ship\'s is: on Windows nothing about a pane\'s '
    + 'foreground can be read, so only a pane a card\'s agent was started in is held back, and a dev '
    + 'server there takes the line like anything else. '
  ...
  projects: 'The first tab along the top is the manager; every tab after it is one project, in the '
    ...
    + 'takes its place, and the next launch opens without it. It refuses while one of that project\'s '
    + 'five shells has anything running in it — an agent, a dev server, a tail, vim — and names each '
    + 'pane in the way with what was found running in it, the way a refused ship does. That is the same '
    + 'reading the command screen picks a free pane by, so a pane it '
    + 'would not type into is a pane this will not kill without asking. The editor is not counted: nvim '
    + 'runs there for as long as the project is open, and a close that counted it could never happen. '
    + 'Nor is a job you put in the background, which leaves the shell at its prompt. So a close it '
