...
const BLURBS: Record<Mode | ActionGroup, string> = {
  ...
  board: 'A kanban board kept in .dashboard/board.json inside the project. Every change is written '
    ...
    + 'There is a Review column as well, fourth from the left, and it is the one column the app moves a '
    + 'card into by itself — the one place a column here is not about what has merged. An agent that '
    + 'writes a pull request number onto its card has finished, so the card lands in Review, the '
