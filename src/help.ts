...
const BLURBS: Record<Mode | ActionGroup, string> = {
  ...
  command: 'One command, run in the projects you mark. Type it once — `npm audit`, `npm outdated`, '
    ...
    + 'into either. An agent, a dev server, a tail, psql, vim — all of them hold a pane back. '
  ...
  projects: 'The first tab along the top is the manager; every tab after it is one project, in the '
    ...
    + 'five shells has anything running in it — an agent, a dev server, a tail, vim — and names the '
    + 'panes in the way. That is the same reading the command screen picks a free pane by, so a pane it '
