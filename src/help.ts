...
const BLURBS: Record<Mode | ActionGroup, string> = {
  ...
  manager: 'The first tab, and the only page that is not a project: no folder and no shells, so the '
    ...
    + 'asking carry the last few lines they printed, so the question can be read from here, and the '
    + 'rest carry the one line they last printed and how long ago it was — which is how you spot the '
    + 'pane that has been quiet since this morning. Enter on a pane takes you '
