...
const BLURBS: Record<Mode | ActionGroup, string> = {
  ...
  manager: 'The first tab, and the only page that is not a project: no folder and no shells, so the '
    ...
    + 'this morning. Enter on a pane takes you '
  ...
  command: 'One command, run in the projects you mark. Type it once — `npm audit`, `npm outdated`, '
    ...
    + 'has not died and has no agent on it, working or waiting for an answer, so the line is never '
    + 'typed at an agent. A pane running an ordinary long command — a dev server, a tail, psql, vim — '
    + 'still counts as free and will take the line: telling a shell prompt from a program that is '
    + 'reading needs shell integration this app does not have. A project with no free pane is named in '
