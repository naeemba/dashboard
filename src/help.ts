...
const BLURBS: Record<Mode | ActionGroup, string> = {
  ...
  command: 'One command, run in the projects you mark. Type it once — `npm audit`, `npm outdated`, '
    ...
    + 'has not died and is not already busy, so the line never lands in an agent mid-answer; a project '
    + 'whose panes are all busy is named in the status bar rather than quietly missed. Nothing comes '
